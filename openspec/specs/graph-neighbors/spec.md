# graph-neighbors Specification

## Purpose

Search surfaces a handful of graph-adjacent nodes automatically, but an agent following up on a
specific hit — "what else calls this," "what does this inherit from," a two- or three-hop trace
— has no way to keep walking the graph from a node ID it already holds. This capability is that
walk: validating a node ID, direction, edge kinds, depth and limit before touching anything;
synchronizing the index exactly as search does; and reading the graph from there.

It owns breadth-first traversal over `calls`, `imports`, `inherits` and `contains` edges that
stops at hubs, external modules and ambiguous-only nodes so it can't sprawl into the whole
repository; deterministic ordering by depth, then edge kind, then confidence, then node ID; a
limit and character budget matching search's; unknown-node errors that suggest current IDs in
the same file; and rendering the result as the same kind of readable, node-ID-bearing text
search produces.
