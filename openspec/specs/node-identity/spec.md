...
A code graph's edges are only as good as the identities its nodes carry: an ID that shifts when
code is merely reformatted or moved without renaming would make every downstream edge unstable,
while scope tracking that only looks at the nearest enclosing class collapses genuinely distinct
symbols — nested functions, JavaScript callbacks, object-literal methods, and Rust trait
implementations — into one colliding identity.

This capability defines the deterministic ID format derived from relative path, full
scope-ancestor chain, name, and kind; the invariants that keep IDs stable across formatting and
location changes while changing on rename or move; the language-specific scope rules that resolve
the known collision cases; the line-ordered `~N` suffixing that disambiguates genuine duplicates;
and the parent linkage that ties every node to its innermost enclosing node.
