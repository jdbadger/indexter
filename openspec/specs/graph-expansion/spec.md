...
Ranked hits alone don't answer "what calls this" or "what breaks if I change it." By M4 the
graph holds resolved `calls`, `inherits`, `imports` and `contains` edges with confidences and
degrees; this capability puts that graph to work in search.

It owns the per-hit graph context shown alongside every entry — up to 3 callers, up to 3
callees, and the containing class — and the one-hop expansion from the top hits into a separate
`related` list: which edges are followed and in which directions, hub damping for
high-degree nodes, the half-weight given to `ambiguous` edges and the rule that a node reached
only through them is dropped, how candidates are scored and ordered, and the reasons attached to
each related node. Related nodes are always additional context, never inserted into the ranking.
