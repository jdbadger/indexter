...
A fused ranking of node IDs isn't a response an agent can act on. This capability shapes what
comes back from a search: collapsing a class's own matched methods into one entry instead of
scattering them across the results, attaching real code — signature, docstring and a snippet
read fresh from disk — to every hit, keeping the whole response inside a character budget
without truncating any single hit, and rendering the result deterministically as grouped,
readable plain text.
