...
Turning a file's raw bytes into the symbols a code graph is built from requires more than running
one language's grammar over it: the right parser must be chosen by extension, tree-sitter queries
compiled once and reused, and the resulting constructs normalized into one vocabulary of kinds
shared across ten different languages plus a chunking fallback for everything else.

This capability owns that pipeline: parser selection and reuse, per-parser query compilation, the
fields every `ParsedNode` carries, the fixed kind vocabulary language constructs normalize into,
the rule that imports and exports produce no nodes, duplicate suppression for wrapped
definitions, and per-file failure reporting that never stops the rest of the repository from
parsing.
