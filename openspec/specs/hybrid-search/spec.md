...
Search is the reason the rewrite exists — letting an agent find code when it can't name the
symbol or module — and by M4 a synced database already holds everything retrieval needs:
composed-summary vectors, a full-text index over names, docstrings and bodies, and a resolved
graph with confidences and degrees. Nothing reads it yet.

This capability owns the search entry point: synchronizing before ranking so results never lag
the working tree, validating queries, filters and limits, retrieving candidates from both vector
and full-text search, applying `kind`/`language`/`path` filters inside both candidate lists,
fusing rankings with Reciprocal Rank Fusion, demoting test files, excluding external-module
nodes, and failing with an actionable error when a repository has no index.
