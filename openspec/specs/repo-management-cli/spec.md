...
This capability provides that: a single `indexter` command group as the shared entry point, and
the `list` and `remove` commands that enumerate and delete database files directly, always reading
state from each database's own stored metadata rather than a registry.
