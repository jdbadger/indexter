# file-walking Specification

## Purpose

Before any file's contents can be parsed or embedded, the system must decide which files in a
repository are worth reading at all — skipping binaries, generated bundles, oversized blobs, and
anything ignored by gitignore-style patterns, without descending into symlinked directories that
escape the repository or falling over on unreadable directories and files that vanish mid-walk.

This capability owns that decision: a synchronous, lazily-iterated walk that yields stat metadata
(not contents) for eligible files, the ignore-pattern and content-independent filtering layers that
decide eligibility, the traversal safety guards, and a separate content-reading step that returns
decoded text together with a path-and-content hash.
