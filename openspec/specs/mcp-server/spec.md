...
After M5, search works and is measured, but only as a Python function — no agent can call it.
This capability is the bridge: an `indexter mcp` command that runs a stdio MCP server exposing
exactly `search` and `neighbors`, with instructions short enough for a client to act on
immediately.

It owns everything between a client's tool call and the underlying search or neighbors core:
resolving which repository a call targets from an explicit argument, the server's own `--repo`,
or its working directory walked upward to the nearest index; loading that repository's settings
fresh on every call; reusing one embedder per embedding configuration for the life of the
process and warming the default one in the background at startup; serializing every call through
one lock so concurrent syncs can't race; turning search, neighbors, configuration, database and
embedding failures into tool errors an agent can act on; and keeping stdout free of anything but
protocol messages.
