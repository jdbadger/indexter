# graph-edges Specification

## Purpose

After M3, `refs` holds every call, import, and inheritance reference with status `unresolved`,
and `edges` is empty — the database has everything vectors need and nothing the graph needs.
Search's "what calls this" and "what breaks if I change it" depend on a graph, not on unresolved
text.

This capability owns what `edges` holds and how it stays consistent: `contains` edges mirrored
from parent links, `calls`/`imports`/`inherits` edges derived from resolved and ambiguous
references with their line and confidence, external-module nodes as import targets,
`nodes.degree` counted from graph edges, and the guarantee that after every sync no edge names a
missing node and the stored edges exactly match what the current nodes and reference outcomes
produce.
