...
After M6 the rewrite works end-to-end but only on the maintainer's machine: nothing enforces the
lint, format, type and coverage gates between commits, and there is no automated check that a
built package actually installs and runs. This capability is the GitHub Actions CI workflow that
runs on every push to `main` and every pull request: a lint job, a test job matrixed over the
supported Python versions with a coverage floor, and a build job that checks the built
distributions' contents and smoke-tests the installed wheel. It owns the workflow's triggers and
concurrency behavior, running jobs against the locked dependencies on a uv-managed interpreter,
and keeping the test suite hermetic — no network access and no model downloads.
