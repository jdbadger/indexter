...
Every other capability needs settings — the embedding model and dimension, storage locations, and
tuning knobs — resolved the same way regardless of whether they come from packaged defaults, a
user's global config, a repository's own config, or an explicit CLI argument.

This capability defines that resolution: a fixed four-layer precedence, strict rejection of
unknown keys and wrong types with an error naming the offending file, and a typed, immutable
settings object as the result, so consumers get attribute access and static type checking instead
of reading a raw dictionary.
