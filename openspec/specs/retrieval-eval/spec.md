...
Search decisions — composed-summary versus raw-code embeddings, hybrid versus vector-only or
keyword-only ranking, test-file demotion, choice of embedding model — need measurement, not
intuition. This capability is the harness that measures them: a fixed question set with expected
files, hit@5 and MRR@10 scoring, a fixed set of comparable variants each built in isolation from
the user's own configuration and databases, a single `just eval` entry point outside CI, and the
recorded rule that decides whether raw-code embeddings replace composed summaries.
